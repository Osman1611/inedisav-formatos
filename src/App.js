    ...
    <div style={{ fontFamily:"'Segoe UI',sans-serif", minHeight:"100vh", background:"#f0f4f8" }}>
      ...
      <div style={{ padding:"20px 24px", maxWidth:1400, margin:"0 auto" }}>
        ...
        {tab === "reportes" && (isAdmin || isCoord) && (()=>{
          // Gather all data
          const todosGrados = GRADOS_ORDEN.filter(g => datosEscuela[g]);
          let allEstudiantes = [];
          todosGrados.forEach(grado => {
            ...
            ests.forEach((est, i) => {
              allEstudiantes.push({
                ...
                estadoFinal: st[i]?.estado || est.estado || "MATRICULADO"
              });
            });
          });

          const total = allEstudiantes.length;

          // By estado
          const porEstado = {};
          ESTADOS.forEach(e => { porEstado[e.key] = 0; });
          allEstudiantes.forEach(e => { if (porEstado[e.estadoFinal] !== undefined) porEstado[e.estadoFinal]++; });

          // By genero
          const hombres = allEstudiantes.filter(e => e.genero !== "F").length;
          const mujeres = allEstudiantes.filter(e => e.genero === "F").length;

          // By SIMAT
          const porSimat = { "REGISTRADO":0, "NO LIBERADO":0, "NO REGISTRADO":0 };
          allEstudiantes.forEach(e => { const s = e.simat||"NO REGISTRADO"; if(porSimat[s]!==undefined) porSimat[s]++; });

          // Con ruta
          const conRuta = allEstudiantes.filter(e => e.ruta && e.ruta.trim()).length;
          const sinRuta = total - conRuta;

          // Retirados list
          const retirados = allEstudiantes.filter(e => e.estadoFinal === "RETIRADO");

          // By grado counts
          const porGrado = todosGrados.map(g => {
            const ests = datosEscuela[g].estudiantes;
            const st = appState?.[g]?.comisiones || {};
            const counts = {};
            ESTADOS.forEach(e => { counts[e.key] = 0; });
            ests.forEach((est,i) => { const estado = st[i]?.estado||est.estado||"MATRICULADO"; if(counts[estado]!==undefined)counts[estado]++; });
            return { grado: g, total: ests.length, ...counts };
          });

          // Filter state
          const [filtroGrado, setFiltroGrado] = React.useState("TODOS");
          const [filtroEstado, setFiltroEstado] = React.useState("TODOS");
          const [filtroGenero, setFiltroGenero] = React.useState("TODOS");
          const [filtroRuta, setFiltroRuta] = React.useState("TODOS");
          const [busqueda, setBusqueda] = React.useState("");

          const rutas = [...new Set(allEstudiantes.map(e=>e.ruta).filter(Boolean))].sort();

          const filtrados = allEstudiantes.filter(e => {
            if (filtroGrado !== "TODOS" && e.grado !== filtroGrado) return false;
            if (filtroEstado !== "TODOS" && e.estadoFinal !== filtroEstado) return false;
            if (filtroGenero !== "TODOS" && e.genero !== filtroGenero) return false;
            if (filtroRuta !== "TODOS" && e.ruta !== filtroRuta) return false;
              ...
              <div style={{ display:"flex", alignItems:"flex-end", gap:6, height:120, padding:"0 4px" }}>
                {data.map((d,i) => (
                  <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
                    <span style={{ fontSize:10, fontWeight:700, color: colors?.[i]||"#1565c0" }}>{d.val}</span>
                    <div style={{ width:"100%", background: colors?.[i]||"#1565c0", borderRadius:"4px 4px 0 0",
                      height: max>0 ? Math.max((d.val/max)*90, d.val>0?6:0) : 0, transition:"height .3s" }}/>
                    <span style={{ fontSize:8, color:"#6b7280", textAlign:"center", lineHeight:1.1 }}>{d.label}</span>
                  </div>
                ))}
              </div>
            );
          ...
          function DonutChart({ segments, size=100 }) {
            ...
            return (
              <svg width={size} height={size} viewBox="0 0 100 100">
                <circle cx={cx} cy={cy} r={r} fill="none" stroke="#f0f4f8" strokeWidth="14"/>
                {segments.filter(s=>s.val>0).map((s,i) => {
                  ...
                  const el = <circle key={i} cx={cx} cy={cy} r={r} fill="none"
                    ...
                    strokeDashoffset={-offset*circ/tot + circ/4}
                    style={{ transition:"stroke-dasharray .4s" }}/>;
                  offset += s.val/tot * circ;
                  return el;
                })}
          ...
          return (
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:10, marginBottom:20 }}>
                <div style={{ background:"linear-gradient(135deg,#1565c0,#0d47a1)", borderRadius:12, padding:"14px 16px", color:"#fff", gridColumn:"span 2" }}>
                  <div style={{ fontSize:11, opacity:.8, textTransform:"uppercase", letterSpacing:1 }}>Total Institución</div>
                  <div style={{ fontSize:36, fontWeight:800 }}>{total}</div>
                  <div style={{ fontSize:11, opacity:.75 }}>♂ {hombres} Hombres · ♀ {mujeres} Mujeres</div>
                </div>
                {ESTADOS.map(e => (
                  <div key={e.key} style={{ background:e.bg, border:`2px solid ${e.color}25`, borderRadius:12, padding:"12px 14px" }}>
                    <div style={{ fontSize:10, fontWeight:600, color:e.color, textTransform:"uppercase" }}>{e.label}</div>
                    <div style={{ fontSize:28, fontWeight:800, color:e.color }}>{porEstado[e.key]}</div>
                    <div style={{ fontSize:11, color:e.color, opacity:.8 }}>{pct(porEstado[e.key])}</div>
                  </div>
                ))}
                <div style={{ background:"#e8f4fd", border:"2px solid #1565c025", borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ fontSize:10, fontWeight:600, color:"#1565c0", textTransform:"uppercase" }}>Con Ruta</div>
                  <div style={{ fontSize:28, fontWeight:800, color:"#1565c0" }}>{conRuta}</div>
                  <div style={{ fontSize:11, color:"#1565c0", opacity:.8 }}>{pct(conRuta)}</div>
                </div>
                <div style={{ background:"#e9ecef", border:"2px solid #49505725", borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ fontSize:10, fontWeight:600, color:"#495057", textTransform:"uppercase" }}>Sin Ruta</div>
                  <div style={{ fontSize:28, fontWeight:800, color:"#495057" }}>{sinRuta}</div>
                  <div style={{ fontSize:11, color:"#495057", opacity:.8 }}>{pct(sinRuta)}</div>
                </div>
              </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14, marginBottom:20 }}>
                ...
                <div style={{ background:"#fff", borderRadius:12, padding:16, boxShadow:"0 1px 6px rgba(0,0,0,.08)" }}>
                  <div style={{ fontWeight:700, fontSize:13, color:"#1e3a5f", marginBottom:12 }}>Distribución por Estado</div>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <DonutChart size={110} segments={ESTADOS.map(e=>({ val:porEstado[e.key], color:e.color }))}/>
                    <div style={{ flex:1 }}>
                      {ESTADOS.map(e=>(
                        <div key={e.key} style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
                          <div style={{ width:10, height:10, borderRadius:2, background:e.color, flexShrink:0 }}/>
                          <span style={{ fontSize:10, color:"#374151", flex:1 }}>{e.label}</span>
                          <span style={{ fontSize:10, fontWeight:700, color:e.color }}>{pct(porEstado[e.key])}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Bar estados */}
                <div style={{ background:"#fff", borderRadius:12, padding:16, boxShadow:"0 1px 6px rgba(0,0,0,.08)" }}>
                  <div style={{ fontWeight:700, fontSize:13, color:"#1e3a5f", marginBottom:12 }}>Estados de Comisión</div>
                  <BarChart
                    data={ESTADOS.map(e=>({ val:porEstado[e.key], label:e.label.slice(0,8) }))}
                ...
                {/* Genero + SIMAT + Ruta */}
                <div style={{ background:"#fff", borderRadius:12, padding:16, boxShadow:"0 1px 6px rgba(0,0,0,.08)" }}>
                  <div style={{ fontWeight:700, fontSize:13, color:"#1e3a5f", marginBottom:10 }}>Género</div>
                  <BarChart data={[{val:hombres,label:"Hombres"},{val:mujeres,label:"Mujeres"}]} colors={["#1565c0","#c01565"]}/>
                  <div style={{ fontWeight:700, fontSize:12, color:"#1e3a5f", marginTop:10, marginBottom:6 }}>SIMAT</div>
                  <div style={{ display:"flex", gap:6 }}>
                    {SIMAT_ESTADOS.map(s=>(
                      <div key={s.key} style={{ flex:1, background:s.bg, borderRadius:8, padding:"6px 8px", textAlign:"center" }}>
                        <div style={{ fontSize:16, fontWeight:700, color:s.color }}>{porSimat[s.key]}</div>
              ...
              {/* ── TABLA POR GRADO ── */}
              <div style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 6px rgba(0,0,0,.08)", marginBottom:20 }}>
                <div style={{ background:"#1565c0", color:"#fff", padding:"10px 16px", fontSize:13, fontWeight:600 }}>
                  Resumen por Grado
                </div>
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                    <thead>
                      <tr style={{ background:"#e8edf5" }}>
                        ...
                        {ESTADOS.map(e=><th key={e.key} style={{...th,fontSize:10,color:e.color}}>{e.label.slice(0,8)}</th>)}
                        <th style={th}>Con Ruta</th>
                      </tr>
                    </thead>
                    <tbody>
                      {porGrado.map((f,ri)=>(
                        <tr key={f.grado} style={{ background:ri%2===0?"#fff":"#f9fbff", borderBottom:"1px solid #e9ecf0" }}>
                          ...
                          {ESTADOS.map(e=>(
                            <td key={e.key} style={{...td,textAlign:"center",
                              background:f[e.key]>0?e.bg:"",color:e.color,fontWeight:f[e.key]>0?700:400}}>
                              {f[e.key]||"–"}
                            </td>
                          ))}
                          <td style={{...td,textAlign:"center",color:"#1565c0",fontWeight:600}}>
                            {datosEscuela[f.grado]?.estudiantes.filter(e=>e.ruta).length||0}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 6px rgba(0,0,0,.08)" }}>
                <div style={{ background:"#1565c0", color:"#fff", padding:"10px 16px", fontSize:13, fontWeight:600 }}>
                  Búsqueda y Filtros — {filtrados.length} estudiantes
                </div>
                <div style={{ padding:"12px 16px", background:"#f8faff", borderBottom:"1px solid #e9ecf0",
                  display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
                  <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="🔍 Buscar por nombre..."
                    style={{ padding:"6px 12px", borderRadius:8, border:"1.5px solid #ddd", fontSize:12, minWidth:180 }}/>
                  <select value={filtroGrado} onChange={e=>setFiltroGrado(e.target.value)}
                    style={{ padding:"6px 10px", borderRadius:8, border:"1.5px solid #ddd", fontSize:12 }}>
                    <option value="TODOS">Todos los grados</option>
                    {todosGrados.map(g=><option key={g} value={g}>{g}</option>)}
                  </select>
                  ...
                  <button onClick={()=>{ setFiltroGrado("TODOS"); setFiltroEstado("TODOS"); setFiltroGenero("TODOS"); setFiltroRuta("TODOS"); setBusqueda(""); }}
                    style={{ padding:"6px 12px", borderRadius:8, background:"#e9ecef", border:"none", cursor:"pointer", fontSize:12, color:"#374151" }}>
                    ✕ Limpiar
                  </button>
                </div>
                <div style={{ overflowX:"auto", maxHeight:400, overflowY:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                    <thead style={{ position:"sticky", top:0 }}>
                      <tr style={{ background:"#e8edf5" }}>
                        <th style={th}>#</th>
                        <th style={{...th,textAlign:"left",minWidth:180}}>Apellidos y Nombres</th>
                        <th style={th}>Grado</th>
                        <th style={th}>Gén.</th>
                        <th style={{...th,minWidth:140}}>Estado</th>
                        <th style={th}>Ruta</th>
                        <th style={th}>SIMAT</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filtrados.map((e,i)=>{
                        const info = estadoInfo(e.estadoFinal);
                        const si = SIMAT_ESTADOS.find(s=>s.key===(e.simat||"NO REGISTRADO"))||SIMAT_ESTADOS[2];
                        return (
                          <tr key={i} style={{ background:i%2===0?"#fff":"#f9fbff", borderBottom:"1px solid #e9ecf0" }}>
                            <td style={{...td,textAlign:"center",color:"#9ca3af"}}>{i+1}</td>
                            <td style={{...td,fontWeight:500}}>{e.nombre}</td>
                            <td style={{...td,textAlign:"center",fontSize:11,color:"#1e3a5f"}}>{e.grado}</td>
                            <td style={{...td,textAlign:"center"}}>{e.genero}</td>
                            <td style={td}>
                              <span style={{ padding:"3px 8px", borderRadius:6, fontSize:11, fontWeight:600,
                                background:info.bg, color:info.color }}>{info.label}</span>
                            </td>
                            <td style={{...td,textAlign:"center",fontSize:11,color:"#1565c0"}}>{e.ruta||"–"}</td>
                            <td style={td}>
                              <span style={{ padding:"2px 6px", borderRadius:5, fontSize:10, fontWeight:600,
                                background:si.bg, color:si.color }}>{si.label}</span>
                            </td>
                          </tr>
                        );
                      })}
                      {filtrados.length === 0 && (
                        <tr><td colSpan={7} style={{ textAlign:"center", padding:24, color:"#9ca3af", fontSize:13 }}>
