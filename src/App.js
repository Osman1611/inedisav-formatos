...
const GRADOS_ORDEN = [
  ...
  "Septimo A","Septimo B","Septimo C",
  "Octavo A","Octavo B",
  "Noveno A","Noveno B",
  "Decimo A","Decimo B",
  "Undecimo A","Undecimo B","Undecimo C",
  "Ciclo 23 A","Ciclo 24 A","Ciclo 25 A"
];
...
const STORAGE_KEY = "inedisav_2026_v3";
...
export default function App() {
  ...
  const [gradoSel, setGradoSel] = useState(GRADOS_ORDEN[0]);
  ...
  const grados = GRADOS_ORDEN.filter(g => DATOS_ESCUELA[g]);
  const gradoData = DATOS_ESCUELA[gradoSel] || { tutor:"", estudiantes:[] };
  const estudiantes = gradoData.estudiantes;

  // Per-student state helpers
  function getEstado(idx) {
    return appState?.[gradoSel]?.comisiones?.[idx]?.estado || estudiantes[idx]?.estado || "MATRICULADO";
  }
  function setEstado(idx, val) {
    update(prev => {
      if (!prev[gradoSel]) prev[gradoSel] = {};
      if (!prev[gradoSel].comisiones) prev[gradoSel].comisiones = {};
      if (!prev[gradoSel].comisiones[idx]) prev[gradoSel].comisiones[idx] = {};
      prev[gradoSel].comisiones[idx].estado = val;
      return prev;
    });
  }
  function getAsistencia(idx, sem, dia) {
    return appState?.[gradoSel]?.asistencia?.[idx]?.[sem]?.[dia] || "";
  }
  function toggleAsistencia(idx, sem, dia) {
    update(prev => {
      if (!prev[gradoSel]) prev[gradoSel] = {};
      if (!prev[gradoSel].asistencia) prev[gradoSel].asistencia = {};
      if (!prev[gradoSel].asistencia[idx]) prev[gradoSel].asistencia[idx] = {};
      if (!prev[gradoSel].asistencia[idx][sem]) prev[gradoSel].asistencia[idx][sem] = {};
      const cur = prev[gradoSel].asistencia[idx][sem][dia] || "";
      prev[gradoSel].asistencia[idx][sem][dia] = cur === "P" ? "F" : cur === "F" ? "" : "P";
      return prev;
    });
  }
  function getNota(idx, periodo, saber) {
    return appState?.[gradoSel]?.notas?.[idx]?.[periodo]?.[saber] ?? "";
  }
  function setNota(idx, periodo, saber, val) {
    update(prev => {
      if (!prev[gradoSel]) prev[gradoSel] = {};
      if (!prev[gradoSel].notas) prev[gradoSel].notas = {};
      if (!prev[gradoSel].notas[idx]) prev[gradoSel].notas[idx] = {};
      if (!prev[gradoSel].notas[idx][periodo]) prev[gradoSel].notas[idx][periodo] = {};
      prev[gradoSel].notas[idx][periodo][saber] = val;
      return prev;
    });
  }
  function getEntrega(idx) {
    return appState?.[gradoSel]?.entregas?.[idx] ?? false;
  }
  function toggleEntrega(idx) {
    update(prev => {
      if (!prev[gradoSel]) prev[gradoSel] = {};
      if (!prev[gradoSel].entregas) prev[gradoSel].entregas = {};
      prev[gradoSel].entregas[idx] = !prev[gradoSel].entregas[idx];
      return prev;
    });
  }

  // Contadores comisiones
  const contadores = {};
  ...
  // Promedio notas
  function calcProm(idx, periodo) {
    const vals = SABERES.map(s => parseFloat(getNota(idx, periodo, s))).filter(n => !isNaN(n));
    if (!vals.length) return "";
    return (vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1);
  }
  function calcPromGeneral(idx) {
    const all = PERIODOS.flatMap(p => SABERES.map(s => parseFloat(getNota(idx, p, s)))).filter(n => !isNaN(n));
    if (!all.length) return "";
    return (all.reduce((a,b)=>a+b,0)/all.length).toFixed(1);
  }

  // Conteo asistencia
  function contarAsistencia(idx) {
    let p=0, f=0;
    SEMANAS.forEach(s => DIAS.forEach(d => {
      const v = getAsistencia(idx, s, d);
      if(v==="P") p++; else if(v==="F") f++;
    }));
    return {p, f};
  }
  ...
  function handlePrint() {
    ...
    const printStyle = `
      <style>
        ...
        .print-header h1 { font-size: 13px; margin: 0 0 2px; }
        .print-header h2 { font-size: 11px; margin: 0 0 2px; font-weight: 600; }
        .print-header p  { font-size: 10px; margin: 0; color: #444; }
        table { width: 100%; border-collapse: collapse; font-size: 9px; }
        th { background: #1565c0 !important; color: #fff !important; padding: 5px 6px;
        ...
        .counter .num { font-size: 18px; font-weight: 700; }
        .counter .lbl { font-size: 8px; font-weight: 600; }
        .check { color: #1a7f37; font-weight: 700; }
        .p-cell { background: #d4edda !important; color: #1e7e34; font-weight: 700; }
        .f-cell { background: #f8d7da !important; color: #721c24; font-weight: 700; }
        .prom-ok { color: #1e7e34; font-weight: 700; }
        .prom-mal { color: #c00; font-weight: 700; }
      </style>
    `;

    let body = "";
    const titulo = tabTitles[tab];
    ...
    if (tab === "comisiones") {
      const rows = estudiantes.map((est, i) => {
        ...
        const checks = ESTADOS.slice(1).map(e =>
          `<td style="text-align:center">${estado===e.key?'<span class="check">✓</span>':'–'}</td>`
        ).join("");
        return `<tr class="${retirado?"ret":""}">
          <td style="text-align:center">${i+1}</td>
          <td>${est.nombre}</td>
          <td style="text-align:center">${est.genero}</td>
          <td style="text-align:center"><span class="badge" style="background:${info.bg};color:${info.color}">${info.label}</span></td>
          ${checks}
        </tr>`;
      }).join("");

      const cntHtml = ESTADOS.map(e =>
        `<div class="counter"><div class="num" style="color:${e.color}">${contadores[e.key]}</div><div class="lbl" style="color:${e.color}">${e.label.toUpperCase()}</div></div>`
      ).join("");

      const headCols = ESTADOS.slice(1).map(e =>
        `<th style="max-width:55px;font-size:8px">${e.label}</th>`
      ).join("");

      body = `
        <div class="counters">${cntHtml}</div>
        <table>
          <thead><tr>
            <th>#</th><th style="text-align:left">Apellidos y Nombres</th><th>Gén.</th><th>Estado</th>${headCols}
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>`;
    }
    ...
    if (tab === "asistencia") {
      const semDiaCols = SEMANAS.flatMap(s =>
        DIAS_LABELS.map(d => `<th>${s}<br/>${d[0]}</th>`)
      ).join("");
      const notaCols = PERIODOS.flatMap(p =>
        SABERES.map(s => `<th>${p}<br/>${s.slice(0,5)}</th>`)
      ).join("");
      const pmCols = PERIODOS.map(p => `<th>${p} PM</th>`).join("");

      const rows = estudiantes.map((est, i) => {
        const {p, f} = contarAsistencia(i);
        const asistCells = SEMANAS.flatMap(s =>
          DIAS.map((_,di) => {
            const v = getAsistencia(i, s, di.toString());
            return `<td class="${v==="P"?"p-cell":v==="F"?"f-cell":""}" style="text-align:center">${v||""}</td>`;
          })
        ).join("");
        const notaCells = PERIODOS.flatMap(per =>
          SABERES.map(sab => `<td style="text-align:center">${getNota(i,per,sab)}</td>`)
        ).join("");
        const pmCells = PERIODOS.map(per => {
          const pm = calcProm(i,per);
          return `<td class="${pm&&parseFloat(pm)>=3?"prom-ok":"prom-mal"}" style="text-align:center">${pm}</td>`;
        }).join("");
        ...
        return `<tr>
          ...
          <td>${est.nombre}</td>
          ${asistCells}
          <td style="text-align:center;font-weight:700;color:#1e7e34">${p}</td>
          <td style="text-align:center;font-weight:700;color:#c00">${f}</td>
          ${notaCells}
          ${pmCells}
          <td class="${pg&&parseFloat(pg)>=3?"prom-ok":"prom-mal"}" style="text-align:center">${pg}</td>
        </tr>`;
      }).join("");

      body = `<div style="overflow-x:auto"><table>
        <thead><tr>
          <th>#</th><th style="text-align:left;min-width:130px">Apellidos y Nombres</th>
          ${semDiaCols}<th>P</th><th>F</th>${notaCols}${pmCols}<th>PM Gral</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table></div>`;
    }
    ...
    if (tab === "entregas") {
      ...
      const rows = estudiantes.map((est, i) => {
        ...
        return `<tr>
          ...
          <td>${est.nombre}</td>
          <td style="text-align:center">${est.genero}</td>
          <td style="text-align:center;font-weight:700;color:${a?"#1e7e34":"#c00"}">${a?"✓ Sí":"✗ No"}</td>
          <td></td>
          <td>${est.telefono||""}</td>
        </tr>`;
      }).join("");
      body = `
        <div class="counters">
          <div class="counter"><div class="num" style="color:#1e7e34">${asistidos}</div><div class="lbl" style="color:#1e7e34">ASISTIERON</div></div>
          <div class="counter"><div class="num" style="color:#c00">${estudiantes.length-asistidos}</div><div class="lbl" style="color:#c00">NO ASISTIERON</div></div>
          <div class="counter"><div class="num" style="color:#004085">${estudiantes.length}</div><div class="lbl" style="color:#004085">TOTAL</div></div>
        </div>
        <table>
          <thead><tr><th>#</th><th style="text-align:left">Apellidos y Nombres</th><th>Gén.</th><th>Asistió</th><th style="min-width:120px">Firma Acudiente</th><th>Teléfono</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>`;
    }

    if (tab === "consolidado") {
      const filas = GRADOS_ORDEN.filter(g => DATOS_ESCUELA[g]).map(grado => {
        const ests = DATOS_ESCUELA[grado].estudiantes;
        const tutor = DATOS_ESCUELA[grado].tutor;
        const st = appState?.[grado]?.comisiones || {};
        const counts = {};
        ESTADOS.forEach(e => { counts[e.key] = { M:0, F:0 }; });
        ests.forEach((est, i) => {
          const estado = st[i]?.estado || est.estado || "MATRICULADO";
          const gen = est.genero === "F" ? "F" : "M";
          if (counts[estado]) counts[estado][gen]++;
        });
        return { grado, tutor, counts,
          totalM: ests.filter(e=>e.genero!=="F").length,
          totalF: ests.filter(e=>e.genero==="F").length,
          total: ests.length };
      });
      const totales = { M:0, F:0, total:0 };
      const totE = {};
      ESTADOS.forEach(e => { totE[e.key] = {M:0,F:0}; });
      filas.forEach(f => {
        totales.M+=f.totalM; totales.F+=f.totalF; totales.total+=f.total;
        ESTADOS.forEach(e => { totE[e.key].M+=f.counts[e.key].M; totE[e.key].F+=f.counts[e.key].F; });
      });

      const headerCols = ESTADOS.map(e =>
        `<th colspan="2" style="background:${e.color};color:#fff;font-size:8px">${e.label}</th>`
      ).join("");
      const subHeaderCols = ESTADOS.map(() => `<th>♂</th><th>♀</th>`).join("");
      const dataRows = filas.map((f,ri) => {
        const cols = ESTADOS.map(e =>
          `<td style="text-align:center;${f.counts[e.key].M>0?`background:${e.bg};color:${e.color};font-weight:700`:""}">${f.counts[e.key].M||"–"}</td>
           <td style="text-align:center;${f.counts[e.key].F>0?`background:${e.bg};color:${e.color};font-weight:700`:""}">${f.counts[e.key].F||"–"}</td>`
        ).join("");
        return `<tr style="${ri%2===0?"":"background:#f5f8ff"}">
          <td style="font-weight:600">${f.grado}</td>
          <td style="font-size:9px">${f.tutor}</td>
          <td style="text-align:center;color:#1565c0;font-weight:700">${f.totalM}</td>
          <td style="text-align:center;color:#c01565;font-weight:700">${f.totalF}</td>
          <td style="text-align:center;font-weight:700">${f.total}</td>
          ${cols}
        </tr>`;
      }).join("");
      const totalRow = ESTADOS.map(e =>
        `<td style="text-align:center;color:#7dd3fc;font-weight:700">${totE[e.key].M||"–"}</td>
         <td style="text-align:center;color:#f9a8d4;font-weight:700">${totE[e.key].F||"–"}</td>`
      ).join("");

      body = `
        <div class="counters">
          <div class="counter"><div class="num" style="color:#1565c0">${totales.total}</div><div class="lbl" style="color:#1565c0">TOTAL</div></div>
          <div class="counter"><div class="num" style="color:#1565c0">${totales.M}</div><div class="lbl" style="color:#1565c0">HOMBRES</div></div>
          <div class="counter"><div class="num" style="color:#c01565">${totales.F}</div><div class="lbl" style="color:#c01565">MUJERES</div></div>
        </div>
        <table>
          <thead>
            <tr style="background:#1e3a5f;color:#fff">
              <th colspan="5" style="text-align:left">Grado / Tutor</th>
              ${headerCols}
            </tr>
            <tr style="background:#e8edf5">
              <th style="text-align:left">Grado</th><th style="text-align:left">Tutor(a)</th>
              <th>♂ H</th><th>♀ M</th><th>Total</th>${subHeaderCols}
            </tr>
          </thead>
          <tbody>
            ${dataRows}
            <tr style="background:#1e3a5f;color:#fff;font-weight:700">
              ...
              <td style="text-align:center;color:#7dd3fc">${totales.M}</td>
              <td style="text-align:center;color:#f9a8d4">${totales.F}</td>
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/>${printStyle}</head><body>
      <div class="print-header">
        <h1>IED SARID ARTETA DE VÁSQUEZ — INEDISAV</h1>
        <h2>${titulo}</h2>
        <p>${tab==="consolidado"?"Año Escolar 2026 · Todos los Grados":`Grado: <strong>${gradoSel}</strong> &nbsp;·&nbsp; Director(a) de Grupo: <strong>${gradoData.tutor}</strong> &nbsp;·&nbsp; Año: 2026`}</p>
      </div>
      ${body}
    </body></html>`);
    win.document.close();
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',sans-serif", minHeight:"100vh", background:"#f0f4f8" }}>
      {/* HEADER */}
      <div style={{ background:"linear-gradient(135deg,#0d47a1,#1565c0)", color:"#fff", padding:"16px 24px" }}>
        <div style={{ fontSize:11, opacity:.75, letterSpacing:2, textTransform:"uppercase" }}>INEDISAV · 2026</div>
        <div style={{ fontSize:20, fontWeight:700, marginTop:2 }}>IED Sarid Arteta de Vásquez</div>
        <div style={{ fontSize:12, opacity:.8, marginTop:2 }}>Sistema de Gestión Académica</div>
      </div>

      {/* SELECTOR GRADO */}
      <div style={{ background:"#fff", borderBottom:"2px solid #e3e8f0", padding:"12px 24px", display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
        ...
        <div style={{ fontSize:13, color:"#6b7280" }}>
          👤 <strong>{gradoData.tutor || "Sin tutor asignado"}</strong>
      ...
      <div style={{ background:"#fff", borderBottom:"2px solid #e3e8f0", padding:"0 24px", display:"flex", gap:4, alignItems:"center" }}>
        {[
          ...
          {key:"asistencia",  label:"📅 Asistencia y Notas"},
          {key:"entregas",    label:"📦 Entrega de Informes"},
        ...
        <div style={{ marginLeft:"auto" }}>
          <button onClick={handlePrint}
            style={{
              padding:"7px 18px", borderRadius:8, border:"1.5px solid #1565c0",
      ...
      <div style={{ padding:"20px 24px", maxWidth:1400, margin:"0 auto" }}>
        ...
        {tab === "comisiones" && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))", gap:10, marginBottom:20 }}>
              {ESTADOS.map(e => (
                <div key={e.key} style={{
                  background: e.bg, border:`2px solid ${e.color}20`,
                  borderRadius:10, padding:"12px 14px", textAlign:"center"
                }}>
                  <div style={{ fontSize:28, fontWeight:700, color:e.color }}>{contadores[e.key]}</div>
                  <div style={{ fontSize:11, fontWeight:600, color:e.color, marginTop:2 }}>{e.label.toUpperCase()}</div>
                </div>
              ))}
            </div>
                ...
                <thead>
                  <tr style={{ background:"#e8edf5" }}>
                    ...
                    {ESTADOS.slice(1).map(e => (
                      <th key={e.key} style={{ ...th, fontSize:10, maxWidth:60, lineHeight:1.2 }}>{e.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {estudiantes.map((est, i) => {
                    ...
                    return (
                      <tr key={i} style={{
                        background: retirado ? "#fff5f5" : i%2===0?"#fff":"#f9fbff",
                        borderBottom:"1px solid #e9ecf0"
                      }}>
                        <td style={{...td, textAlign:"center", color:"#9ca3af"}}>{i+1}</td>
                        <td style={{...td, fontWeight: retirado?400:500, color: retirado?"#dc3545":"#1e293b",
                          textDecoration: retirado?"line-through":"none"}}>
                          {est.nombre}
                        </td>
                        ...
                        <td style={{...td}}>
                          <select
                            value={estado}
                            onChange={e => setEstado(i, e.target.value)}
                            style={{
                              width:"100%", padding:"5px 8px", borderRadius:6, fontSize:12, fontWeight:600,
                              border:`1.5px solid ${info.color}40`, background:info.bg, color:info.color,
                              cursor:"pointer"
                            }}
                          >
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                <thead>
                  <tr style={{ background:"#e8edf5" }}>
                    ...
                    {SEMANAS.map(s => (
                      DIAS.map((d,di) => (
                        <th key={s+d+di} style={{...th,fontSize:9,maxWidth:28,padding:"4px 2px"}}>
                          {s}-{DIAS_LABELS[di].slice(0,1)}
                        </th>
                      ))
                    ))}
                    ...
                    {PERIODOS.map(p => SABERES.map(s => (
                      <th key={p+s} style={{...th,fontSize:9,maxWidth:36,lineHeight:1.2}}>
                        {p}<br/>{s.slice(0,5)}
                      </th>
                    )))}
                  ...
                  {estudiantes.map((est, i) => {
                    ...
                    return (
                      <tr key={i} style={{ background:i%2===0?"#fff":"#f9fbff", borderBottom:"1px solid #e9ecf0" }}>
                        ...
                        {SEMANAS.map(s => DIAS.map((d,di) => {
                          const val = getAsistencia(i, s, di.toString());
                          return (
                            <td key={s+di} style={{...td,textAlign:"center",padding:"2px 1px"}}>
                              <button
                                onClick={() => toggleAsistencia(i, s, di.toString())}
                                style={{
                                  width:24, height:22, border:"1px solid #ddd", borderRadius:4,
                                  fontSize:10, fontWeight:700, cursor:"pointer",
                                  background: val==="P"?"#d4edda":val==="F"?"#f8d7da":"#fff",
                                  color: val==="P"?"#1e7e34":val==="F"?"#721c24":"#9ca3af"
                                }}
                              >{val||"·"}</button>
                            </td>
                          );
                        ...
                        {PERIODOS.map(per => SABERES.map((sab,si) => (
                          <td key={per+si} style={{...td,padding:"2px 1px"}}>
                            <input
                              ...
                              value={getNota(i, per, sab)}
                              onChange={e => setNota(i, per, sab, e.target.value)}
                              style={{
                                width:36, padding:"2px 3px", border:"1px solid #ddd",
                                borderRadius:4, fontSize:11, textAlign:"center", background:"#fff"
                              }}
                            />
                          </td>
                        )))}
                        {PERIODOS.map(per => (
                          <td key={"pm"+per} style={{...td,textAlign:"center",fontWeight:600,
                            color: parseFloat(calcProm(i,per))>=3?"#1e7e34":"#dc3545",
                            background:"#f0f4f8"}}>
                            {calcProm(i,per)}
                          </td>
                        ))}
                        <td style={{...td,textAlign:"center",fontWeight:700,background:"#e8edf5",
                          color: parseFloat(calcPromGeneral(i))>=3?"#1e7e34":"#dc3545"}}>
                          {calcPromGeneral(i)}
                        </td>
                      </tr>
                    );
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:12, marginBottom:16 }}>
              {[
                ...
                <div key={c.label} style={{ background:c.bg, border:`2px solid ${c.color}30`,
            ...
            <div style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 6px rgba(0,0,0,.08)" }}>
              <div style={{ background:"#1565c0", color:"#fff", padding:"10px 16px", fontSize:13, fontWeight:600 }}>
                ASISTENCIA REUNIÓN PADRES DE FAMILIA · {gradoSel} · Director(a): {gradoData.tutor}
              </div>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                <thead>
                  <tr style={{ background:"#e8edf5" }}>
                ...
                <tbody>
                  {estudiantes.map((est, i) => {
                    const asistio = getEntrega(i);
                    return (
                      <tr key={i} style={{ background: asistio ? "#f0fff4" : i%2===0?"#fff":"#f9fbff",
                        borderBottom:"1px solid #e9ecf0" }}>
                        <td style={{...td, textAlign:"center", color:"#9ca3af"}}>{i+1}</td>
                        <td style={{...td, fontWeight:500}}>{est.nombre}</td>
                        <td style={{...td, textAlign:"center", color:"#6b7280"}}>{est.genero}</td>
                        <td style={{...td, textAlign:"center"}}>
                          <button
                            onClick={() => toggleEntrega(i)}
                            style={{
                              padding:"5px 16px", borderRadius:20, border:"none", cursor:"pointer",
                              fontWeight:700, fontSize:12,
                              background: asistio ? "#d4edda" : "#f8d7da",
                              color: asistio ? "#1e7e34" : "#721c24"
                            }}
                          >{asistio ? "✓ Sí" : "✗ No"}</button>
                        </td>
                        <td style={{...td}}>
                          {est.telefono
                            ? <a href={`tel:${est.telefono}`} style={{ color:"#1565c0", textDecoration:"none" }}>
                                📞 {est.telefono}
                              </a>
                            : <span style={{ color:"#d1d5db" }}>–</span>}
                        </td>
        ...
        {/* ══════════════ CONSOLIDADO FINAL ══════════════ */}
        {tab === "consolidado" && (() => {
          // Build consolidated data across ALL grades
          const filas = GRADOS_ORDEN.filter(g => DATOS_ESCUELA[g]).map(grado => {
            const ests = DATOS_ESCUELA[grado].estudiantes;
            const tutor = DATOS_ESCUELA[grado].tutor;
            const st = appState?.[grado]?.comisiones || {};
            ...
            const totalM = ests.filter(e => e.genero !== "F").length;
            const totalF = ests.filter(e => e.genero === "F").length;
            return { grado, tutor, counts, totalM, totalF, total: ests.length };
          });

          // Column totals
          const totales = { M:0, F:0, total:0 };
          const totEstados = {};
          ESTADOS.forEach(e => { totEstados[e.key] = { M:0, F:0 }; });
          filas.forEach(f => {
            totales.M += f.totalM; totales.F += f.totalF; totales.total += f.total;
            ESTADOS.forEach(e => {
              totEstados[e.key].M += f.counts[e.key].M;
              totEstados[e.key].F += f.counts[e.key].F;
            });
          });
                ...
                {ESTADOS.slice(1).map(e => (
                  <div key={e.key} style={{ background:e.bg, border:`2px solid ${e.color}20`, borderRadius:10, padding:"12px 16px", textAlign:"center" }}>
                    <div style={{ fontSize:22, fontWeight:700, color:e.color }}>
                      {totEstados[e.key].M + totEstados[e.key].F}
                    </div>
                    <div style={{ fontSize:10, fontWeight:600, color:e.color }}>{e.label.toUpperCase()}</div>
                    <div style={{ fontSize:10, color:e.color, opacity:.7 }}>
                      ♂{totEstados[e.key].M} · ♀{totEstados[e.key].F}
                    </div>
                  </div>
                ))}
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                  <thead>
                    <tr style={{ background:"#1e3a5f", color:"#fff" }}>
                      ...
                      {ESTADOS.map(e => (
                        <th key={e.key} colSpan={2} style={{...th, fontSize:10, background:e.color, color:"#fff", minWidth:80}}>
                          {e.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {filas.map((f, ri) => (
                      <tr key={f.grado} style={{ background:ri%2===0?"#fff":"#f9fbff", borderBottom:"1px solid #e9ecf0" }}>
                        ...
                        {ESTADOS.map(e => [
                          <td key={e.key+"m"} style={{...td, textAlign:"center", background: f.counts[e.key].M>0?e.bg:"", color:e.color, fontWeight: f.counts[e.key].M>0?700:400}}>
                            {f.counts[e.key].M || "–"}
                          </td>,
                          <td key={e.key+"f"} style={{...td, textAlign:"center", background: f.counts[e.key].F>0?e.bg:"", color:e.color, fontWeight: f.counts[e.key].F>0?700:400}}>
                            {f.counts[e.key].F || "–"}
                          </td>
                        ])}
                      </tr>
                    ...
                    <tr style={{ background:"#1e3a5f", color:"#fff", fontWeight:700 }}>
                      ...
                      {ESTADOS.map(e => [
                        <td key={e.key+"m"} style={{...td, textAlign:"center", color:"#7dd3fc", fontWeight:700}}>
                          {totEstados[e.key].M || "–"}
                        </td>,
                        <td key={e.key+"f"} style={{...td, textAlign:"center", color:"#f9a8d4", fontWeight:700}}>
                          {totEstados[e.key].F || "–"}
                        </td>
                      ])}
                    </tr>
...
const th = {
  padding: "8px 10px", fontWeight:700, fontSize:12, color:"#374151",
  borderRight:"1px solid #e9ecf0", textAlign:"center", whiteSpace:"nowrap"
};
const td = {
  padding: "6px 10px", borderRight:"1px solid #f0f4f8", verticalAlign:"middle"
};
